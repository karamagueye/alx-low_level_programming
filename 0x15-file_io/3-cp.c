...
/**
 * main - funct copie the content of one file to another
...
int main(int argc, char *argv[])
{
	...
	unsigned int mode = S_IRUSR | S_IWUSR | S_IWGRP | S_IWGRP | S_IROTH;
	char buffer[1024];

	if (argc != 3)
	{
		dprintf(STDERR_FILENO, "%s", "usage: cp file_ffrom file_to\n");
		exit(97);
	}
	...
	close_src = close(arc);
	check_IO_stat(close_src, arc, NULL, 'C');
